myTrend_LM<-function(dt,n_boot,id_col,start_column){
  ...
  for(i in 1:nrow(dt)){
    ...
    t1[is.na(t1)] = 0
    # bootstrap
    t_res <- myFun_Bootstrap_LM(t1,n_boot,2)
    dt_trend[i,] <- data.frame(t_res)
    #dt_trend = clusterR(t1, myFun_Bootstrap)
    
  }
  
  colnames(dt_trend) = c("fao_id","ratio_1st","p_value_1st","ratio_2nd","p_value_2nd")
  dt_trend <- as.tibble(dt_trend)
  
  dt_trend$ratio_1st[dt_trend$p_value_1st > 0.1] <- 0
